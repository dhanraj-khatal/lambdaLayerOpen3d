...
class Easy_Mesh(object):
    ...
    def get_point_curvatures_normalized(self, method='mean'):
        curv = vtk.vtkCurvatures()
        curv.SetInputData(self.vtkPolyData)
        if method == 'mean':
            curv.SetCurvatureTypeToMean()
    ...
    def get_cell_curvatures(self, method='mean'):
        ...
        self.cell_attributes['Curvature'] = (np.clip(
            np.mean(tmp_cell_curvts, axis=-1).reshape([tmp_cell_curvts.shape[0], 1]), -3, 3) + 3) / 6
    ...
    def get_ordered_boundary_points(self):
        ...
        return boundary_points_per_polygon
